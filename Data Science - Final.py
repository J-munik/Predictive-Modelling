...
CHUNKSIZE = 300000
reader = pd.read_csv('/Users/jaquelyn/Desktop/train_sample.csv', header=0, engine='c', chunksize=CHUNKSIZE)
...
test_reader = pd.read_csv('/Users/jaquelyn/Desktop/test.csv', header=0, engine='c', chunksize=CHUNKSIZE)
...
def decision_boundary(prob):
    return 1 if np.any(prob >= .5) else 0
...
def classify(preds):
    ...
    #Get Predictions
    predicted_values
    decision_boundary2 = np.vectorize(decision_boundary)
    preds = pd.DataFrame()
    preds = decision_boundary2(predicted_values).flatten()
    return(preds)

preds=classify(predicted_values)
print(preds)
...
##create results array_classification
preds = preds.transpose(1,0)
preds = np.asarray(preds)
predicted_class = np.asarray(tuple(preds))
preds_class = pd.DataFrame(predicted_class, columns = ['is_attributed_classification'])
####create results array_prediction
...
results = pd.concat([click_id, pred, preds_class], axis = 1)


##Publish results as CSV

with open('/Users/jaquelyn/Desktop/Submission.csv','w') as output:
    writer = csv.writer(output, lineterminator='\n')
    writer.writerows([['click_id','is_attributed','is_attributed_classification']])
